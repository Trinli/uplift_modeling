...
def visualize_auuc():
    ...
    #ax21.set_ylabel(r'$\mathbf{E}(\mathrm{y})$')
    ...
    ax12.set_ylabel(r'$\mathbb{E}(\mathrm{y})$')
...
def plot_similarly_refined():
    ...
    # y-axis represents p(\mathrm{y}=1 \vert \mathbf{x})
    ...
    ax12.plot(x_0, p_0, label=r'$p(\mathrm{y}=1 \vert \mathbf{x}, w=1)$')
    ax12.plot(x_1, p_1, label=r'$p(\mathrm{y}=1 \vert \mathbf{x}, w=0)$')
    ax12.set_xlabel('x')
    ax12.set_xticks([0.4])
    ...
    ax21.plot(x_2, p_2, label=r'$p(\mathrm{y}=1 \vert \mathbf{x}, w=1)$')
    ax21.plot(x_3, p_3, label=r'$p(\mathrm{y}=1 \vert \mathbf{x}, w=0)$')
    ax21.set_xlabel('x')
    ax21.set_yticks([])
...
def plot_similarly_refined_3():
    ...
    ax12.plot(x_0, p_0, label=r'$p(\mathrm{y}=1 | x, w=1)$')
    ax12.plot(x_3, p_3, label=r'$p(\mathrm{y}=1 | x, w=0)$')
    ax12.set_xlabel('x')
    ...
    ax21.plot(x_2, p_2, label=r'$p(\mathrm{y}=1 | x, w=1)$')
    ax21.plot(x_3, p_3, label=r'$p(\mathrm{y}=1 | x, w=0)$')
    ax21.set_xlabel('x')
    ax21.set_ylim([0, 1])
    ax21.set_yticks([])
    ...
    # Add boxes with green transparent to indicate positive uplift and red transparent to indicate negative.
    # ((x-coord, y-coord), width, height)
    patch_tmp = patches.Rectangle((0.0, 0.3), 0.2, 0.05, linewidth=0, facecolor="#00FF0020")
    ax12.add_patch(patch_tmp)
    patch_tmp = patches.Rectangle((0.2, .35), 0.2, 0.35, linewidth=0, facecolor="#FF000020")
    ax12.add_patch(patch_tmp)
    patch_tmp = patches.Rectangle((0.4, 0.7), 0.3, 0.1, linewidth=0, facecolor="#00FF0020")
    ax12.add_patch(patch_tmp)
    patch_tmp = patches.Rectangle((0.7, .55), 0.3, 0.25, linewidth=0, facecolor="#00FF0020")
    ax12.add_patch(patch_tmp)

    patch_tmp = patches.Rectangle((0.0, 0.15), 0.2, 0.15, linewidth=0, facecolor="#FF000020")
    ax21.add_patch(patch_tmp)
    patch_tmp = patches.Rectangle((0.2, 0.5), 0.5, 0.2, linewidth=0, facecolor="#FF000020")
    ax21.add_patch(patch_tmp)
    patch_tmp = patches.Rectangle((0.7, 0.55), 0.3, 0.4, linewidth=0, facecolor="#00FF0020")
    ax21.add_patch(patch_tmp)
