...
class UpliftDataset(object):
    ...
    data_format : dict
        A dictionary containing necessary details of the dataset. 
        uplift_modeling._data_formats contains ready made dicts for the most popular
        uplift datasets. See example below for the Voter-dataset (Gerber, Green, &
        Larimer, 2008).
...
def quick_load(dataset='hillstrom'):
    ...
    """
    import uplift_modeling._data_formats as _data_formats
    if dataset == 'hillstrom':
        print("Loading the Hillstrom dataset.")
        return UpliftDataset("./datasets/" + _data_formats.HILLSTROM_FORMAT['file_name'],
                                _data_formats.HILLSTROM_FORMAT)
    elif dataset == 'criteo':
        print("Loading subset of the Criteo-dataset with 100k observations.")
        return UpliftDataset("./datasets/criteo_100k.csv", _data_formats.CRITEO_FORMAT)
    elif dataset == 'voter':
        print("Loading the Voter-dataset.")
        return UpliftDataset('./datasets/' + _data_formats.VOTER_FORMAT['file_name'],
                                 _data_formats.VOTER_FORMAT)
    elif dataset == 'lenta':
        print("Loading mini-version of Lenta-dataset.")
        return UpliftDataset("./datasets/lenta_mini.csv", _data_formats.LENTA_FORMAT)
    elif dataset == 'starbucks':
        print("Loading the Startbucks-dataset.")
        return UpliftDataset('./datasets/' + _data_formats.STARBUCKS_FORMAT['file_name'],
                                 _data_formats.STARBUCKS_FORMAT)
    else:
